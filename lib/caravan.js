...
export const ADDR = {
  caravan: '0x04bBCc7a1EF4c999e1dFCe94D4C502eD5549e84a', // CaravanMarket forwarder (emits CaravanPurchase)
...
const CARAVAN_FROM_BLOCK = 18467077;
...
// live price of 1 share in USDG (dollars), read from the pool via the static quoter
export async function getPrices() {
  const provider = new JsonRpcProvider(CHAIN.rpc);
  const q = new Contract(ADDR.quoter, QUOTER_ABI, provider);
  const one = parseEther('1');
  const out = {};
  await Promise.all(Object.entries(STOCKS).map(async ([sym, s]) => {
    try {
      const path = solidityPacked(['address', 'uint24', 'address'], [s.addr, s.fee, ADDR.USDG]);
      const usdg = await q.quoteExactInput(path, one);
      out[sym] = Number(formatUnits(usdg, 6));
    } catch { out[sym] = null; }
  }));
  return out;
}
...
const ROUTER_ABI = ['function exactInput((bytes path,address recipient,uint256 amountIn,uint256 amountOutMinimum)) payable returns (uint256)'];
const CARAVAN_ABI = [
  'function buy(bytes[] calls, string basket) payable',
  'event CaravanPurchase(address indexed buyer, string basket, uint256 ethIn, uint256 timestamp)',
];

// One transaction through the forwarder: swaps run inside, tokens land in the
// buyer's wallet, and a verifiable CaravanPurchase event is emitted.
export async function buyBasket({ signer, user, basket, ethAmount, slippagePct, quotes, onStep }) {
  ...
  const caravan = new Contract(ADDR.caravan, CARAVAN_ABI, signer);
  const tx = await caravan.buy(calls, basket.name, { value: total });
  await tx.wait();
  return tx.hash;
}

// Verifiable purchase history, read straight from the chain's CaravanPurchase events.
export async function readPurchases(limit = 25) {
  const provider = new JsonRpcProvider(CHAIN.rpc);
  const c = new Contract(ADDR.caravan, CARAVAN_ABI, provider);
  const latest = await provider.getBlockNumber();
  const events = await c.queryFilter(c.filters.CaravanPurchase(), CARAVAN_FROM_BLOCK, latest);
  return events.reverse().slice(0, limit).map(e => ({
    buyer: e.args.buyer,
    basket: e.args.basket,
    eth: formatUnits(e.args.ethIn, 18),
